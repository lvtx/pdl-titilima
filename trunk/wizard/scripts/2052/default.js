...
function OnFinish(selProj, selObj) {
	try	{
		...
		var bConsole = wizard.FindSymbol("CONSOLE_APP");
		if (!bConsole) {
		    wizard.AddSymbol("RC_FILE_NAME", CreateSafeRCFileName(strProjectName) + ".rc");
		}

		selProj = CreateProject(strProjectName, strProjectPath);
		ConfigForDebug(selProj, "Debug", charSetUNICODE);
		ConfigForRelease(selProj, "Release", charSetUNICODE);
...
function ConfigForDebug(oProj, strName, chSet) {
	try	{
		var config = oProj.Object.Configurations(strName);
        config.CharacterSet = chSet;
...
function ConfigForRelease(oProj, strName, chSet) {
	try	{
		var config = oProj.Object.Configurations(strName);
        config.CharacterSet = chSet;
...
function GetTargetName(strName, strProjectName, strResPath, strHelpPath)
{
	try	{
		...
		} else if ("main" == strName.substr(0, 4)) {
